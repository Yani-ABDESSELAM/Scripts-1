...
begin {
  function Get-DnsHostname ($IPAddress) {
    ...
    $Result | Where-Object { $_ -match 'name' } | ForEach-Object {
      $_.Replace('Name:    ', '')
    }
  }
...
process {
  try {
    ...
    $StaleRecords = Get-DnsServerResourceRecord -ComputerName $DnsServer -ZoneName $DnsZone -RRType A | Where-Object { $_.TimeStamp -and ($_.Timestamp -le (Get-Date).AddDays("-$StaleThreshold")) -and ($_.Hostname -like "*.$DnsZone") }
